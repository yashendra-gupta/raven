...
declare -r DOWNLOAD_URL_JDK_15="https://download.java.net/java/GA/jdk15.0.2/0d1cfde4252546c6931946de8db48ee2/7/GPL/openjdk-15.0.2_linux-x64_bin.tar.gzz"
...
do_switch () {
    ...
    if [[ -e $jdk_package_cache_location ]]; then
        ...
        local unpack_location=$JDK_UNPACK_FILE_LOCATION\/$version_to_switch

        if [[ ! -e $jdk_package_unpack_location ]]; then
            echo "Unpacking JDK $version_to_switch, please wait..."

            tar -xf $jdk_package_cache_location -C $unpack_location --strip-components=1 # https://linuxize.com/post/how-to-extract-unzip-tar-gz-file/
            echo "unpacked JDK $version_to_switch successfully !!"
        fi

        local jdk_symlink="/opt/development/bin/jdk"
        ln -sfn $unpack_location $jdk_symlink
